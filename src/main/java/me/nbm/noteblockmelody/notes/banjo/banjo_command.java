...
public class banjo_command implements CommandExecutor {

    private final NoteBlockMelody plugin;

    public banjo_command(NoteBlockMelody plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (player.hasPermission("nbm.use")) {
            banjo_event event = new banjo_event(plugin);
            event.openBanjoGUI(player);
        } else {
