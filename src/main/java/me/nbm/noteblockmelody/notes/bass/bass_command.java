...
public class bass_command implements CommandExecutor {

    private final NoteBlockMelody plugin;

    public bass_command(NoteBlockMelody plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (player.hasPermission("nbm.use")) {
            bass_event event = new bass_event(plugin);
            event.openBassGUI(player);
        } else {
