...
public class didgeridoo_command implements CommandExecutor {

    private final NoteBlockMelody plugin;

    public didgeridoo_command(NoteBlockMelody plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (player.hasPermission("nbm.use")) {
            didgeridoo_event event = new didgeridoo_event(plugin);
            event.openDidgeridooGUI(player);
        } else {
