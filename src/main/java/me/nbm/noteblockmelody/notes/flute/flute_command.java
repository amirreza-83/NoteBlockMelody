...
public class flute_command implements CommandExecutor {

    private final NoteBlockMelody plugin;

    public flute_command(NoteBlockMelody plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (player.hasPermission("nbm.use")) {
            flute_event event = new flute_event(plugin);
            event.openFluteGUI(player);
        } else {
